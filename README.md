...
| 评测报告 | 按运行汇总全部对手、换边结果、M1 分布与 Web 历史列表 |
...
同一 checkpoint 在本地 MPS 上重新评测得到 788 胜、212 负，胜率 78.8%，平均
M1 为 9.024，无平局或截断。该结果位于同一 run 的
`eval_vs_f1d2_mps.json`，用于验证 M1 和报告链路，不替代上表 CUDA 正式基准。
M1 定义为终局双方剩余总生命之差的绝对值，加存活角色数之差的绝对值。
...
默认监听 `0.0.0.0:51731`。对局入口为 `http://127.0.0.1:51731/`，评测历史入口为
`http://127.0.0.1:51731/reports`。
...
MPS 可在 macOS 主机进程中使用，但当前模型较小且 F1D2 搜索、对局推进均为串行工作，
1000 局端到端实测为 271.50 秒，慢于先前约 192 秒的 CPU 运行。当前建议小规模评测
使用 CPU；批量训练或推理结构改变后再重新基准测试 MPS。

指定 `--output` 时保留完整 JSON。Web 服务按运行目录生成一份在线报告，并在同一页汇总
该运行对全部对手的评测。每次评测包含核心结果、先后手胜率、换边后的先手必胜/后手
必胜/双方各自必胜、平均 M1 和带四分位线的 M1 胜负分布折线图。图表使用
ECharts，四分位使用 simple-statistics。
...
当前状态和接手信息分别见 `docs/0_status/README.md` 与 `docs/HANDOFF.md`。
