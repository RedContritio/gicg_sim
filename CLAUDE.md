...
走 `tools/runs/*` ssh wrappers,**绝不手写 `ssh` / `scp` / `tar | ssh`**。**每 cfg 自闭包含 host 决策** — 训练 `cfg.toml` 加 `[meta].host = "remote"` + `[remote]` 段(`ssh / root / os / hostname`,如 `configs/dmc/stage3_b_v_legacy.toml`);工具按 cfg dispatch local vs remote(loopback = `socket.gethostname() == [remote].hostname`),第一 positional 接 cfg。无 file-based config / env override(P2 2026-05-21 砍)。Production 序列:`tools.runs.kill <cfg> --all`(清残留 python)→ `git commit` → `tools.runs._remote_sync <cfg>`(默认 since-last-sync:uncommitted ∪ diff(remote_sha..HEAD);首次 → 全 ls-files;clean tree 推进 remote `.last_synced_sha`)→ `tools.runs.build_engine <cfg>`(改了 `gicg_engine/` 才需;probe go+gcc PATH)→ `tools.runs._ssh <cfg> --stream -- .venv/Scripts/python.exe -X utf8 -u -m tools.runs.train <cfg> > /tmp/<label>.log 2>&1 &`(`--stream` Popen 实时透传长跑必用)→ 另 shell `tools.runs.tail <cfg> artifacts/<run>/metrics.jsonl --follow`(`--lines N` non-follow / 任意远程文件)→ 训练完 `tools.runs.pull <cfg> <run-label>`(默认 ckpts/latest + metrics + tb;`--all-ckpts` 拉全 / `--dir <p>` 整 dir / `--files <glob>`)。其它:`_remote_sync --dry-run` / `--git-changed`(legacy uncommitted-only)/ `--tar-all` / `--single <p>` / `--from-sha <S>`;`kill --pid <N>` / `--match <substr>` / `--dry-run`。远端 venv (`.venv|venv|env|.virtualenv`) + `go` / `gcc` 全 PATH probe,缺失 fail loud(`discover_remote_python` / `discover_remote_binary`)。
