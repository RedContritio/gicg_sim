...
.venv/bin/python -m tools.runs.sync push|pull <config.toml>                      # rsync metadata + cfg backup(NOT ckpts/)
...
## Remote training (cfg-driven dispatch)

走 `tools/runs/*` wrappers,**绝不手写 `ssh` / `scp` / `tar | ssh`**。**每 cfg 自闭包 host 决策** — 训练 `cfg.toml` 加 `[meta].host = "remote"` + `[remote].profile = "<hosts.toml profile>"`;device profile 保存在 gitignored 的 `configs/hosts/hosts.toml`(`ssh / root / os / hostname` 4 字段)。工具按 cfg dispatch local vs remote(loopback = `socket.gethostname().lower() == [remote].hostname.lower()`),第一 positional 接 cfg。无 file-based config / env override(P2 2026-05-21 砍)。P3 后 `tools.runs.train <cfg>` 是 atomic 单命令入口:本地 cfg 走 Phase A→B→C 原 lifecycle,remote cfg 内部 auto-sync + ssh forward 远端跑(stream stdout/stderr 实时透传,本地 Ctrl+C → SIGTERM 远端 proc 防 leak),artifacts/ 在远端,本地仅 dispatcher。Production 序列:`tools.runs.kill <cfg> --all`(清残留 python)→ (optional) `git commit`(default sync 是 `--auto`:uncommitted ∪ diff,未 commit 也能 sync;clean tree 才推进 remote `.last_synced_sha`)→ `tools.runs.build_engine <cfg>`(改了 `gicg_engine/` 才需;probe go+gcc PATH)→ `tools.runs.train <cfg> &`(atomic — 内部 auto-sync + ssh_forward stream)→ 另 shell `tools.runs.tail <cfg> artifacts/<run>/metrics.jsonl --follow`(`--lines N` non-follow / 任意远程文件)→ 训练完 `tools.runs.pull <cfg> <run-label>`(默认 ckpts/latest + metrics + tb;`--all-ckpts` 拉全 / `--dir <p>` 整 dir / `--files <glob>`)。`_remote_sync` / `_ssh` **internal / diagnostic only**(P3 后被 train 内部调用,user 不直接 invoke);需要显式远端命令时用 `tools.runs.exec <cfg> -- <argv...>`。远端 venv (`.venv|venv|env|.virtualenv`) + `go` / `gcc` 全 PATH probe,缺失 fail loud(`discover_remote_python` / `discover_remote_binary`)。**配 cfg 前必先 `.venv/bin/python -m tools.runs.exec <cfg> -- hostname` 验 hostname 实际值**。
