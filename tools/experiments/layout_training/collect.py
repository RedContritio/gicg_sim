...
def collect(cfg, checkpoint, scenarios, seed, layout_count=5):
    agents = [builder(cfg, checkpoint)(seed) for _ in range(layout_count)]
    ...
    for index, spec in enumerate(specs):
        for side in (0, 1):
            envs = [factory(index, layout_seed=derive_seed(seed, f'layout-{k}', index)) for k in range(layout_count)]
