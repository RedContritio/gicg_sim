...
def run(config, panel_path, index, side, output):
    ...
    game_cfg = with_teams(ev._CFG, case.team_0, case.team_1)
    rule_seed = derive_seed(seed, 'rule-variant', index)
    with environment_config(game_cfg, panel.get('variants'), rule_seed, index, split='heldout') as (game_cfg, _):
        factory = make_env_factory(game_cfg, None, seed)
