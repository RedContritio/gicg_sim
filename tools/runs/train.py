"""tools.runs.train — single-command atomic lifecycle (T-08 Phase A).

This module is the eventual sole entry point for training under the
2026-05-18 ``tools/runs/`` clean-slate redesign (spec
``docs/superpowers/specs/2026-05-18-tools-runs-redesign-design.md``
§Architecture 行 24-74).

Phase A (this task T-08) implements lifecycle steps 0-3 only:

- step 0  validate ``cfg.meta.run_label`` regex (post --override resolve)
- step 1  capture leaf cfg bytes (read_bytes() immediately, races immune)
- step 2  resolve cfg = ``load_with_extends`` + ``--override`` apply
- step 2.5 ``artifacts/`` mkdir (parents=True, exist_ok=True)
- step 3  allocate NNN under flock + ``mkdir`` per-run dir O_EXCL with
          ``EEXIST`` retry outer loop

Phase B (T-09) writes ``cfg_leaf.toml`` + ``cfg_resolved.toml`` +
``metadata.toml`` (status='running'); phase C (T-10) runs train and
closes metadata. T-08 ``main()`` prints a stub message and exits 0 once
phase A completes.
