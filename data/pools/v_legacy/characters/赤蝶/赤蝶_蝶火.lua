...
-- Mirror-match fix (#152): capture the owning slot at load time.
-- Every actor-centric hook below filters ctx to this owner so the
-- buff effect doesn't double-apply when both players run this file
-- (赤蝶 vs 赤蝶).
...
on_round_end_decay(function(ctx)
  ...
  -- our counter isn't active to avoid wasted work.
  if 蝶火_active:get() > 0 then
    蝶火_rounds:sub(1)
