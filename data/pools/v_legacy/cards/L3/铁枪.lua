local ref = declare_card("铁枪", { dices = { any = 2 } }, { target = "own", requires_weapon = Weapon.Polearm })
local equipped = get_counter("equipped", Scope.PerChar)
local last_skill = declare_counter("铁枪_last_skill", Scope.PerChar, nil, { ref_kind = RefKind.Skill })
...
on_damage_add({ order = equipped }, function(ctx)
  ...
  if ls ~= nil and ls == ctx.skill_index then
