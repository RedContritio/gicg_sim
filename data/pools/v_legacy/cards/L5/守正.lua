...
local 泼墨 = get_counter("泼墨", Scope.PerPlayer)
...
-- 守正下水龙吟额外先造成 2 水伤害
on_skill_use({ order = active }, function(ctx)
...
-- 泼墨被消耗时获得正气
on_after_write(泼墨, Op.Sub, { order = active }, function(ctx)
  if active:get() <= 0 then return end
  正气:add(1)
end)

-- 水云被消耗前：正气替代
on_before_write(水云, Op.Sub, { order = 正气 }, function(ctx)
  if active:get() <= 0 then return end
...
-- 正气回合结束衰减（召唤物结算之后）
on_round_end_decay({ order = 正气 }, function(ctx)
  if 正气:get() > 0 then
    正气:sub(1)
  end
end)
...
register_buff(正气, { remove_on_death = true })
