local card = declare_card("兽肉薄荷卷", { dices = { match = 1 } }, { target = "own" })
local food = get_counter("饱腹", Scope.PerChar)
local uses = declare_counter("兽肉薄荷卷_次数", Scope.PerChar, 0, { min = 0, max = 3 })
...
on_card_play(function(ctx)
  ...
  uses:set_at(ctx.target_player, ctx.target_char, 3)
end)
