...
def test_ppo_payload_header_size_matches_go():
    """Layout size 跟 Go side PpoTransitionHeader binary.Size 一致(56 byte)。

    PpoTransitionHeader:ChosenAction(4) + StepInEp(4) + RewardX1M(4) + LogProbX1M(4)
    + ValueX1M(4) + 5×u32 N* (20) + StaticHash(16) = 56 byte。
    """
    from training.core.actor.transition_sink_wire import _PPO_PAYLOAD_HEADER_SIZE

    assert _PPO_PAYLOAD_HEADER_SIZE == 56
