...
| I29 | **Go-native actor pool — eliminate per-actor Python process** | session 2026-05-20 共识。当前 N=16 actor 每个 Python 进程 547 MB,16×=8.7 GB actor RSS;每 inference 经 pickle + IPC ~ms 级 RTT。Go actor 重写:N goroutine in 1 Go binary,env / obs encode / IPC client 全 Go-native,inference 走单 socket 到 Python InfServer(同当前)。**收益**:(a) 节 8.7 GB actor RSS → goroutine 几 KB 各,N=64/128 可行;(b) in-process channel ~µs vs pickle ~ms IPC;(c) DSL 只 load 一次共享;(d) actor 自然 inference batching dispatcher(SEED RL / Acme 架构)。**不解决**:InfServer mem 漏(独立问题,平台限制)。**Scope**:~2000 Go LOC + ~500 Python edit(InfServer accept Go conn / Trainer Go transition stream)。port:obs encoder(`_capture_obs_np` / `_encode_static_np` → Go),InferenceClient,Transition buffer,EpisodeRunner main loop,opp baseline,spec sampler。**Risk**:numpy → Go array math 移植要数值 equivalence test;现 5 paradigm 共享 core/actor 抽象,改造意味 paradigm-agnostic boundary 重切。**Action**:production 跑稳 + Mac gauntlet 验证 + 1 轮 baseline 数据后,dispatch subagent 做 design ADR(类似 IR-1.5 audit phase),设计稳后再 implementation。 | 2 | idea (2026-05-20 session 共识) |
| I28 | **`tools.remote` 完整 workflow — 消除手动 ssh/scp** | session 2026-05-20 实测 `sync` + `run` + `pull` + `status` 覆盖 ~70% 用例,但仍需要手动 ssh 的场景:① kill 远程 python 进程(`Stop-Process` 反复手动)② tail 任意远程文件(actor_log / metrics 自定义文件 tail)③ 通用 dir pull(`tools.remote.pull` 只支持 ckpts,perf_logs / 任意 dir 拉不了)。**+ 2 broken**:`build_engine.py` gcc 路径写 `C:\msys64\mingw64\bin`(Win box 实际 Strawberry `C:\Strawberry\c\bin\gcc.exe`)+ double-quote 被 ssh 吞 / `status.py` line 30 `REMOTE = 'dev@192.168.31.56'` hardcoded 没用 `_common` config。**Fix scope**(~175 LOC):新 `tools.remote.kill`(~30 LOC,`--all` / `--pid <N>` / `--match <name>`)+ `tools.remote.tail`(~50 LOC,`--lines N` / `--follow` 任意远程文件)+ `pull --dir <remote_path>` / `--files <glob>` 扩(~60 LOC)+ status.py 用 config(~5 LOC)+ build_engine.py 修(~20 LOC)+ CLAUDE.md 加 kill/tail/pull doc(~10 LOC)。**完成后真正 zero manual ssh/scp**。 | 2 | idea (2026-05-20 session 实测发现) |
