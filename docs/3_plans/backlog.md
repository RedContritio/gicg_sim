...
| I24 | **DMC mp actor 移除 torch 依赖** | actor 不跑网络前向(forward 在 InferenceServer GPU 上),但 `_DMCObsDictRemoteProvider.forward` + `DMCEpisodePolicy.act` 用 torch CPU 把 numpy obs_dict 转成 tensor 才发给 server。这一步纯机械式转换,完全可以让 server 端做。actor 移除 `import torch` 可省 ~1 GB CUDA libs RSS / actor(cu130 build):8-actor 省 ~8 GB,16-actor 省 ~16 GB。当前 InfServer 23 GB + 8×1.5 GB actor = ~35 GB,改造后估 ~25 GB → 16-actor 即使 mem 紧也能跑。**Why**:bench 2026-05-20 显示 4-actor / 16-actor 都吞吐 ~38 fps,但 16-actor 内存逼近 64 GB 上限(24-actor 已 swap thrash → 20 fps 倒退)。actor 瘦身后 16-actor 不再受 mem 制约 + 启动更快。**How**:`_DMCObsDictRemoteProvider.forward` 改用 numpy 计算 obs_dict;`DmcAgent.build_obs_dict` 拆出纯-numpy 版本(static cache 用 numpy array 不是 torch tensor);InfServer 收到 numpy obs_dict 后 `torch.from_numpy` 转成 batch tensor。`_episode.py:capture_obs` 已经返回 numpy → 这条路径已经 work,build_obs_dict 是 torch path 是 legacy。 | 2 | idea (2026-05-20 bench finding) |
