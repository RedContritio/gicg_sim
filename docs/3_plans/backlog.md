...
| I28 | **`tools.runs` 完整 workflow — 消除手动 ssh/scp** | session 2026-05-20 实测 `sync` + `run` + `pull` + `status` 覆盖 ~70% 用例,但仍需要手动 ssh 的场景:① kill 远程 python 进程(`Stop-Process` 反复手动)② tail 任意远程文件(actor_log / metrics 自定义文件 tail)③ 通用 dir pull(`tools.runs.pull` 只支持 ckpts,perf_logs / 任意 dir 拉不了)。**+ 2 broken**:`build_engine.py` gcc 路径写 `C:\msys64\mingw64\bin`(Win box 实际 Strawberry `C:\Strawberry\c\bin\gcc.exe`)+ double-quote 被 ssh 吞 / `status.py` line 30 `REMOTE = 'dev@192.168.31.56'` hardcoded 没用 `_common` config。**Fix scope**(~175 LOC):新 `tools.runs.kill`(~30 LOC,`--all` / `--pid <N>` / `--match <name>`)+ `tools.runs.tail`(~50 LOC,`--lines N` / `--follow` 任意远程文件)+ `pull --dir <remote_path>` / `--files <glob>` 扩(~60 LOC)+ status.py 用 config(~5 LOC)+ build_engine.py 修(~20 LOC)+ CLAUDE.md 加 kill/tail/pull doc(~10 LOC)。**完成后真正 zero manual ssh/scp**。 | 2 | idea (2026-05-20 session 实测发现) |
| I31 | **AZ/CFR/PPO mp actor pool 统一到 `core/actor.actor_main`** | W2-4 deferred (2026-05-28 session):W2-1~W2-3+5+6 ship 后 audit 第一波 #4(paradigm 各自重新发明 mp actor pool)依然 open。 现状:DMC 已迁(`paradigms/dmc/mp_factories.py` 317 LOC + `core/actor.runtime.spawn_pipeline + actor_main`,I29 R7 ship 的产物);AZ `inference_pool.py` (236) + `inference_worker.py` (156) = 392 LOC,CFR `parallel_trainer.py` (170) + `worker.py` (203) = 373 LOC,PPO `_async.py` 300 LOC 仍各自维护。 **Scope**:per paradigm 写 ~300 LOC `mp_factories.py`(`build_env_factory` / `build_opp_registry` / `build_policy` / `build_provider` / `spec_sampler` 五件套)+ rewrite 各自 collector dispatch 走 `core/actor.runtime.spawn_pipeline` + `actor_main` + 删 inference_pool/worker/parallel_trainer/worker/_async + 各 paradigm test 迁(每 paradigm 1 mp e2e test 含 weight sync + deadlock + episode lifecycle)。 **Risk**:mp race / weight sync / deadlock 是 latent bug 重灾区,3 paradigm 各自 episode lifecycle 不同(AZ selfplay+arena / CFR traversal session / PPO rollout),不能一刀切。 **建议**:3 个独立 OpenSpec change(`change/az-mp-pool-unification` / `cfr-mp-pool-unification` / `ppo-mp-pool-unification`),按 paradigm 优先级排序(PPO 首,因 _async 单文件最简单 + 是 mp 模板;CFR 次,因 frozen-research tier 收益低 但模板复用大;AZ 末,因 mp 路径复杂含 arena 二级 spawn)。 每 change ~1 周 wall + smoke_full 验证 | 3 | deferred (W2-4 backlog, 2026-05-28) |
