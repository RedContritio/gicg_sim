# 暂停期三次会话记录（2026-09-14 ×2、2026-09-15）

> 从 `docs/HANDOFF_PAUSED.md` 移出的会话叙事，作历史存档。当前状态见
> [实时入口](../0_status/README.md)，恢复顺序见 [暂停交接](../HANDOFF_PAUSED.md)。
> 未启动过任何 56 训练；所有产出都是本机代码 + 本机 smoke + 行数清理。
...
同会话另新增 change `remote-host-decoupling`（已定稿未执行）。**未启动训练**；改动已提交到
`dev`，尚未 push。
...
被拆分所致），也**不等于**更早的 `c2af135c…` / `b3e84a0a…`。后果：
...
### 新增 change：remote-host-decoupling（已定稿未执行）

把 `[remote]` 的 `ssh` / `os` / `hostname` / `root` 四字段从 14 个 cfg 外置到 gitignored 的
...
用户 2026-09-15 说明 `D:/gicg_goal` / `D:/gicg_native` / `D:/gicg_consequence_20260914` /
`D:/gicg_retention_20260915` 已手工删除，只保留 `D:/gicg_dev`（设备关机，本机无法核验）。
实验之间的隔离靠 cfg 参数与 per-run 目录，**不靠另开根** —— 每个根各持一套 `artifacts/` 索引
与 NNN 计数器，多根会让 `show <NNN>` 不再唯一。
...
- ⚠️ 同批记录里**三处已失效**：`必要文件` 表的「六份关键模型备份及来源/SHA」一行、
  「从 `warmup.pt` 出发」、以及「远端来源指纹 `c2af135c…`」。
...
| `configs/dmc/policy_retention.toml` | — | 56 生产配置（当时指向的远端根已手工删除，现统一为 `D:/gicg_dev`） |
...
### 行数清理（该会话中断时的进度，已由 2026-09-15 会话完成）
...
**批 1 剩余 3 个**（当时未动）：`gicg_actor/dmc/paradigm_test.go`(529)、
`gicg_actor/shm/shm_test.go`(334)、`gicg_engine/interp/ir/audit_test.go`(531)。
...
- 该时点 `git status` 为 **92 modified + 36 untracked**，含全部拆分产物、6 个 ruff 格式化、
  5 个 gofmt 格式化。这些已在 2026-09-15 会话按逻辑单元提交到 `dev`，**尚未 push**；远端
  `git@github.com:RedContritio/gicg_sim.git` 仍是 `dev` = `1e12d97`、`main` = `22c7345`、
  tag `v0.3.0` → `1e12d97`，即上游仍是本轮之前的状态。
...
上一轮 56 根为 `D:/gicg_consequence_20260914`，Python `D:/gicg_native/venv/Scripts/python.exe`；
两个根均已手工删除（现统一为 `D:/gicg_dev`），故这两条路径已失效。远端来源指纹
`c2af135c…`（**已失效**）。所有远程操作使用 `tools.runs._host`。
