...
- [x] **T0.1** 创建 `configs/hosts/hosts.toml`(gitignored),写入 `[gpu-win]`
...
- [x] **T0.2** 确认 `.gitignore` 新增 `configs/hosts/hosts.toml`,且
      `git status` 不显示该文件。— LOC 1 — 依赖:T0.1

## Phase 1 — 解析层

- [x] **T1.1** `configs/hosts/hosts.example.toml`(tracked 模板,占位符值,
      含 `root = "D:/project"`)。— LOC ~9(新增)— 依赖:无
- [x] **T1.2** `tools/runs/_host.py`:新增 `HOST_REGISTRY` 常量 +
...
- [x] **T1.3** `tools/runs/_host.py` docstring:第 3–17 行的示例块改用
      `profile` + 占位符形式,去掉真实 IP。— LOC ~15(modify)— 依赖:T1.2
- [x] **T1.4** `tools/runs/_remote_sync.py`:`_auto_sync` 构造 `paths` 处
...
- [x] **T1.5** **新建** `tools/runs/tests/test_host_registry.py`,把
      ...
      净迁移)— 依赖:T1.2
- [x] **T1.6** 改 `tools/experiments/tests/test_native_starter_config.py:18`:
      断言 `load_remote_from_cfg(...).ssh == 'dev@192.168.31.56'` 改为基于 tmp
      注册表 fixture 断言 profile 解析结果。— LOC ~10 — 依赖:T1.2
- [x] **T1.7** **确认**(非修改)`RemoteCfg` 数据类形状不变(仍是
...
- [x] **T2.1** 14 个 `configs/dmc/*.toml` 的 `[remote]` 段:四字段
...
- [x] **T3.1** `docs/` — **已在 docs 重构中顺带完成(2026-09-15,先于本 change
      ...
      — LOC ~13 — 依赖:无
- [x] **T3.2** `openspec/changes/archive/` **3 个文件 4 处**:
      ...
      — LOC ~6 — 依赖:T2.1
- [x] **T3.3** `training/paradigms/dmc/PLAN.md:735,744,746`(3 处)、
      `notes.md:343,429,432,477,480`(5 处)。— LOC ~10 — 依赖:T2.1
- [x] **T3.4** `tools/_bench/p2_results/` **12 个文件**,全部在第 5 行、形状统一为
...
- [x] **T3.5** 其余代码内标识:`tools/eval/daemon.py:15,138` 与
      ...
      **`tools/runs/tests/test_schema.py:31`**(`host='Mac-mini.local'` 测试
      ...
      — LOC ~10 — 依赖:T2.1
- [x] **T3.6** `tools/_bench/run_win_collector_pair.py`:删 `_WIN_SSH`(:40)/
      `_WIN_ROOT`(:41)常量,改注册表解析(`--host-profile`,默认 `gpu-win`);
      :3 的 docstring 与 :158 的输出行同步。— LOC ~20 — 依赖:T1.2
- [x] **T3.6b** `configs/dmc/eval_stage3_b_v_legacy.toml:8` —— 该文件**没有**
      `[remote]` 段,唯一命中是注释里的示例命令,故不在 T2.1 的 14 个之列,
      在此单独替换。— LOC ~1 — 依赖:T2.1
- [x] **T3.7** `git grep` 复核:三项标识在 HEAD 内零残留(区分大小写;同时
...
- [x] **T4.1** 本机:`pytest -n 4 gicg_env/tests/ training/tests/ tools/runs/tests/
      tools/eval/tests/` 全绿。— 依赖:T1–T3
- [x] **T4.2** 四项 pre-commit 检查:`check_line_limits` 零违规
      (`tools/_meta/check_line_limits.py` 全仓审计)、`ruff format --check`、
      `gofmt -l` 无输出、`check_openspec_indices` 退出 0。— 依赖:T1–T3
- [x] **T4.3** 确认 `fingerprint()` 仍为
      ...
      — 依赖:T4.2
- [x] **T4.4** 确认**无需**改文档指纹记录(依据 T4.3 的指纹中性主张)。
      ...
      — LOC 0 — 依赖:T4.3
- [ ] **远端首次同步与训练派发全链路验收(设备开机后)**:`tools.runs.train
      ...
      (工具类 spec 止步于 mock 是 anti-pattern)。授权已完成,当前仅依赖设备可用。
      — 依赖:T4.1–T4.4 + 设备可用

## 验收出口

`T4.1`–`T4.4` 可在设备关机期间完成;远端首次同步与训练派发全链路验收不可。
**在该验收通过之前,本 change
