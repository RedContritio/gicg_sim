...
- Q01/Q02/Q03/Q05 已裁定并在 v3 实现；下一批处理 Q04 与剩余覆盖缺口，之后
  再做随机对局、失败序列保存与最小化。
...
- `TestBuffInstances_FIFOAndLifecycle`：乘胜/茶点两种产生顺序、双席位、快照/克隆/checkpoint。
- `TestBuffInstances_LotusPrecedesWaterCloud`：两种顺序都由荷花酥归零且保留水云。
- `TestBuffInstances_TeamInkSurvivesOwnerDeath`：泼墨生产者死亡后队伍仍触发独立伤害。
- `TestBuffInstances_DeathClearsMarkWithoutTransfer`：标记角色死亡不转移、不误击替补。
- `TestBuffObservationLiveOrderAndEnemyMark`：实例顺序、进度与敌方标记可见、恢复一致。
- `test_buff_learning.py`：规则引用重排不变性、实际顺序敏感性、空列表、共享编码器梯度、
  适配参数原样保留、只追加原语行、混采比例及组合留出。
